...
if args.load_model_variance:
    model_variance = tsm.networks.VarianceEstimator.load(args.load_model_variance, device)
else:
    model_variance = tsm.networks.VarianceEstimator(inshape, nb_unet_features=[enc_nf, dec_nf])
...
if args.variance_loss_type == 'beta-NLL':
    ...
    loss_func_variance = tsm.losses.VarianceNLL(beta=args.beta).loss
...
if ndims == 3:
    losses_motion += [tsm.losses.Grad('l2', loss_mult=args.int_downsize).loss]
elif ndims == 2:
    losses_motion += [tsm.losses.Grad_2d('l2', loss_mult=args.int_downsize).loss]
else:
        ...
        if flag_motion == True:
            ...
            for n, loss_function in enumerate(losses_motion):
                ...
                if n < len(losses_motion) - 1:
                    ...
                    # image_weight, weight_sigma, weight_I = vxm.losses.compute_weight_from_variance(y_true[n], logsigma_image, y_pred[-1]) # （y_true, logsigma_image, mask_bgd）
                    image_weight = tsm.losses.compute_SNR_from_variance(y_true[n], logsigma_image, y_pred[-1]) # （y_true, logsigma_image, mask_bgd）

                    image_weight_vis.append(image_weight)
                    # weight_sigma_vis.append(weight_sigma)
                    # weight_I_vis.append(weight_I)
...
model_motion.save(os.path.join(model_dir, 'tsm_{}_motion.pt'.format(str(args.dataset))))
model_variance.save(os.path.join(model_dir, 'tsm_{}_variance.pt'.format(str(args.dataset))))
