...
class LoadableModel(nn.Module):
    ...
    def load(cls, path, device):
        ...
        # Modify the configuration to remove deprecated keys
        if "logvar_norm" in checkpoint['config']:
            del checkpoint['config']['logvar_norm'] 
        if "sampling" in checkpoint['config']:
            del checkpoint['config']['sampling']
