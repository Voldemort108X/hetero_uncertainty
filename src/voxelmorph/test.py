...
model_variance = vxm.networks.VarianceEstimator(inshape=inshape).load(args.model_variance, device)
...
for file_path in tqdm(test_files):
    ...
    image_weight, weight_sigma, weight_I = vxm.losses.compute_weight_from_variance(input_ES, logsigma_image, mask_bgd)
    ...
    if args.dataset == 'Echo':
        save_file = {
                ...
                "weight_sigma": weight_sigma.detach().cpu().numpy()[0,0],
                "weight_I": weight_I.detach().cpu().numpy()[0,0]
                }
